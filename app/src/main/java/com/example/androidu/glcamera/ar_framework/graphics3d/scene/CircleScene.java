...
public class CircleScene extends Scene{
    ...
    private ArrayList<Entity3D> mCircleTransformList = new ArrayList<>();
    ...
    public void add(Entity3D entity){
        super.add(entity);
        mCircleTransformList.add(new Entity3D());
    }

    public void update(){
        for(int i = 0; i < mEntityList.size(); i++){
            /* find bearing from mCenterCoord to e */
            Entity3D entity = mEntityList.get(i);
            ...
            Entity3D entity2 = mCircleTransformList.get(i);
            entity2.reset();
    ...
    public void draw(float[] projectionMatrix, float[] viewMatrix){
        for(int i = 0; i < mEntityList.size(); i++){
            Entity3D entity1 = mEntityList.get(i);
            Entity3D entity2 = mCircleTransformList.get(i);
